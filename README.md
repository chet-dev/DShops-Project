...
$ ganache-cli
```
Important note: You would need to copy the ganache-cli generated Mnemonic 12 words to a text file, so that you can use them (cut and paste) to restore the test accounts on your Metamask Dapp browser, in order to test the web app locally on port 8545.
...
Note: You can note down the deployed DShops.sol contract address manually for troubleshooting later, if necessary. This contract address should be the same as the one in **DShops.abi.js** from **Step 3** of [Test The Frontend Web App in dshopsvue subfolder](#Test-The-Frontend-Web-App-in-dshopsvue-subfolder).
...
If you are using Metamask Dapp browser, you should select **localhost 8545** (ganache) and import the Mnemonic 12 words generated by ganache-cli from **Step 5** of [**Test The Smart Contract in DShops subfolder**](#Test-The-Smart-Contract-in-DShops-subfolder). The first account should be the Contract Owner / Admin of DShops.

You can refer to the [**project demo video**](https://youtu.be/TPgQgAMCCuc) to test the web app yourself.
