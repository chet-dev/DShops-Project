...
contract DShops is Pausable {
    ...
    modifier onlyStoreOwnerOfProduct(bytes32 productCode) {
        require(productExists(productCode), "product does not exist!");
        require(
            msg.sender == storefronts[products[productCode].storefrontUniqueId].storeOwner,
            "msg.sender must be the store owner of the product!"
        );
        _;
    }

    modifier onlyStoreOwnerOfStorefront(bytes32 storefrontUniqueId) {
        ...
        require(
            ...
            "msg.sender must be the store owner of the storefront!"
        );
        _;
    }
    ...
    function getAdminByIndex(uint256 index) public view returns (address adminAddr) {
        require(adminIndexes.length > 0, "Operation aborted. No admins to retrieve");
        adminAddr = adminIndexes[index];
        return (adminAddr);
    }
    ...
    ) public whenNotPaused onlyStoreOwnerOfStorefront(storefrontUniqueId) returns (uint256 index) {
        // require(storefrontExists(storefrontUniqueId), "storefront does not exist!");
        // require(
        //     storefronts[storefrontUniqueId].storeOwner == msg.sender,
    ...
    function updateProductPrice(bytes32 productCode, uint256 newPrice)
        public
        whenNotPaused
        onlyStoreOwnerOfProduct(productCode)
    {
        //require(productExists(productCode), "Product does not exist!");
    ...
    function deleteProduct(bytes32 productCode)
        public
        whenNotPaused
        onlyStoreOwnerOfProduct(productCode)
    {
